...
DELETE FROM talebeler WHERE id='126';--on delete cascade yadıgım icin silebildim.
--parent chil iliskisinden silemedim ama one delete cascade yardimiyla sildim
-- on deletegormeden tekrar olusturursam silemem 
...
DROP TABLE if exists musteriler-- atblo olusturulmadan once yazmak iyidir
...
SELECT round(avg(maas),2) AS maas_ortalaması FROM calisanlar2;--virgulden sonra 2 sayiyi aliyor
...
null da olsa sayiyor ama maas yazarsak null gormez * kullanirsak null i da goror
*/
...
SELECT * FROM maxminmaas;--bununla method olusturdum
select * from summaas
...
/*
UPDATE tablo_id
SET sutun_isim = 'istenen veri' WHERE sutun_isim='istenen veri'
*/
UPDATE tedarikciler
