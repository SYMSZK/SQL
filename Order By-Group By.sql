...
-- Insanlar tablosundaki tum kayitlari isimler Natural sirali(ASC), Soyisimler ters sirali olarak listeleyin
...
--text yapti brilestirdi

SELECT isim||' '||soyisim AS isim_soyisim FROM insanlar1 ORDER BY LENGTH (isim)+LENGTH (soyisim)

SELECT isim||' '||soyisim AS isim_soyisim FROM insanlar1 ORDER BY LENGTH (isim_soyisim)
--yukaridaki isim icin hata verir gecici isim oldugu icin 
...
SELECT COUNT(isim) FROM manav --urun sayisi

SELECT urun_adi, count(isim) AS urunden_alan_kisi_sayisi FROM manav
GROUP BY urun_adi;

--isime gore alinan urun
